...
function RewardsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Unable to load your rewards right now.');
  ...
  const redeem = async (reward) => {
    try {
      ...
      const token = await shopify.sessionToken.get();
      const key = `account:${reward._id}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
      const response = await fetch(`${apiBase}/api/customer/redemptions/reserve`, {method: 'POST', headers: {Authorization: `Bearer ${token}`, 'Content-Type': 'application/json', 'Idempotency-Key': key}, body: JSON.stringify({rewardId: reward._id})});
  ...
  const next = useMemo(() => (data?.rewards || []).find((r) => Number(r.pointsCost) > points), [data, points]);
  const progress = next ? Math.min(100, Math.round((points / Number(next.pointsCost || 1)) * 100)) : 100;

  return <s-page heading={title}><s-stack direction="block" gap="base">
